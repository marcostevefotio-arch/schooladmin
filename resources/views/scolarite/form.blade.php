...
@section('content')
    <div class="row">
        <div class="col-xs-12">
            <form action="{{ isset($data)? route("scolariteUpdate", ["slug"=>$data->id]): route("scolariteStore") }}" method="post">
                {{ csrf_field() }}
                ...
                <div class="form-group">
                    ...
                    <select name="compte" id="compte" class="form-control" required>
                        <option value=""></option>
                        @foreach($compte as $c)
                            <option value="{{ $c->id }}"  {{  isset($data)? ($data->compte_id==$c->id? "selected" : "") : "" }}>{{ $c->libelleCompte }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    <label for="montantScolarite">Montant *</label>
                    <input type="number" name="montantScolarite" id="montantScolarite" min="0" class="form-control form-control-sm" value="{{  isset($data)? $data->montantScolarite : old("montantScolarite") }}" required style="letter-spacing: 20px; font-weight: bold"/>
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" rows="5" id="motif" class="form-control form-control-sm">{{  isset($data)? $data->descriptionScolarite : old("description") }}</textarea>
                </div>
