...
@section('content')
    <div class="row">
        <div class="col-xs-12">
            <form action="{{ route("start-annee") }}" method="post">
                {{ csrf_field() }}

                <div class="form-group">
                    <label for="numeroAnnee">Année academique *</label>
                    <select name="numeroAnnee" id="numeroAnnee" class="form-control @error('numeroAnnee') is-invalid @enderror" value="{{ old('numeroAnnee') }}" required>
                        <option value=""></option>
                        @foreach($avalable as $i)
                ...
                <h4><button type="submit" id="submit" class="btn btn-primary"><i class="fa fa-save">&nbsp;Demarrer l'année</i></button></h4>
            </form>
        </div>
    </div>
@endsection
...
@section("script")
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            $("#anneeacademique").chosen({
                disable_search_threshold: 10,
                no_results_text: "Oops, nothing found!",
                width: "100%"
            });

            $('#anneeacademique').on('change', function(evt, params) {
                $.ajax({
